...
_BaslerGigECamera::_BaslerGigECamera() : _GenericCamera()
{
...
_BaslerGigECamera::~_BaslerGigECamera()
{
	if (device != nullptr)
	{
		if (device->IsOpen())
...
std::vector<std::string> _BaslerGigECamera::listAvailableDevices(void)  throw(std::exception)
{
...
bool _BaslerGigECamera::initialize(const std::string id)  throw(std::exception)
{
	if (device != nullptr)
	{
		detach();
...
bool _BaslerGigECamera::reset() throw(std::exception)
{	
	this->detach();
	return this->initialize(this->id);
}

void _BaslerGigECamera::detach(void)  throw(std::exception)
{
...
TimedFrame _BaslerGigECamera::getTimedFrame(void) throw (std::exception)
{
	...
	try
	{
		...
		return TimedFrame(cvImage.clone(), double(timestamp));
	}
	catch (GenericException& e)
	{
...
cv::Mat _BaslerGigECamera::getFrame(void)  throw(std::exception)
{
	//std::cout << "start: " << std::chrono::system_clock::now().time_since_epoch().count() << std::endl;
	if (device == nullptr)
	{
...
bool _BaslerGigECamera::setExposureMode(ExposureModes mode)  throw(std::exception)
{
...
bool _BaslerGigECamera::setExposure(const double time)  throw(std::exception)
{
	if (device != nullptr)
	{
		try
...
double _BaslerGigECamera::getExposure(void) const  throw(std::exception)
{
	if (device != nullptr)
	{
		try
...
void _BaslerGigECamera::loadFeatureFile(const std::string filename) throw(std::exception)
{
	if (device != nullptr)
	{
		try
